...
public:
    ...
    void CacheScopes(bool filter_usable = false) {
        ...
        // bases --- type_id of subclass : [parent classes]
        ...
        // deriveds --- type_id of baseclass : [subclasses]
        ...
        for (size_t i = 0; i < _schema["scopes"].size(); ++i) {
            ...
            for (unsigned int conn_idx = 0; conn_idx < def.in_ref.size(); ++conn_idx) {
                ...
                // conn_idx no change since this indicates index of argument of the function consumed
    ...
    void helper(unsigned int type_id, std::vector<int> done) {
        if (done[type_id] != 1) {
            ...
            // make sure all derived classes processed
            for (size_t i=0; i < sub.size(); ++i) {
                unsigned int s = sub[i];
                helper(s, done);
            }
            // append derived class' initializer
            for (size_t i=0; i < sub.size(); ++i) {
                unsigned int s = sub[i];
    ...
    bool Validate(bool debug=false, unsigned int max_depth=5, bool skip_validation=false, bool prune_cache=false) {
        ...
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            ...
            // Tests only the first one which is generated with original implementation, not the ones that belong to derived-class and added to base-class
    ...
    int con_des(const char* filepath) {
        ...
        outfile << "IsConstructor(i)" << std::endl;
