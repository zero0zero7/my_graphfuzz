...
class CPPScope(object):
    def __init__(self, name: str, inputs: List[str], outputs: List[str], 
            args: List[str], exec_str: str, weights: dict):
        self.name = name
        self.inputs = inputs
    ...
    def from_dict(info: dict) -> 'CPPScope':
        ...
        weights = dat.get('weights') or {}

        return CPPScope(name, inputs, outputs, args, exec_str, weights)
    ...
    @staticmethod
    def from_signature(schema: Schema, parent: dict, sig: CPPFuncSignature, is_static: bool, generate_dry: bool = False, weights: dict = {}) -> 'CPPScope':
    ...
    for k in schema.objects:
        ...
        if obj['type'] in ['class', 'struct']:
            ...
            for i in range(len(sigs)):
                ...
                try:
                    scope = CPPScope.from_signature(schema, obj, psig, is_static, generate_dry=generate_dry, weights=weights[i])
...
def make_shim_json(typ_dict: dict, scp_list: List):
    ...
    for i in range(len(scp_list)): 
        scope = scp_list[i]
        shim_in[int(i)] = []
        for j in range(len(scope.inputs)):
            shim_in[int(i)].append(typ_dict[scope.inputs[j]])
        if len(scope.inputs) == 0: 
            if len(scope.outputs) > 0: # scope is constructor
                shim_in[int(i)].append(typ_dict[scope.outputs[0]])
    with open("shim.json", "w") as outfile:
        json.dump(shim_in, outfile)
...
def make_fuzz_schema(schema: Schema, scopes: List[CPPScope]) -> dict:
    ...
    # dictionary --- name : id
    # Excludes abstract classes (since they are not in schema.yaml)
    type_name_id = {}
    for d in types:
        type_name_id[d["name"]] = d["id"]

    bases = []
    deriveds = []
    for k in schema.objects:
        if schema.objects[k]['type'] == 'struct':
            b = schema.objects[k]['base']
            d = schema.objects[k]['derived']
            bases.append({'id': schema.objects[k]['id'],
                        'child': [type_name_id[name] for name in b if name in type_name_id.keys()]})
            deriveds.append({'id': schema.objects[k]['id'],
                        'parent': [type_name_id[name] for name in d if name in type_name_id.keys()]})

    id_name = {} # id : class name
    for name in type_name_id:
        id_name[type_name_id[name]] = name
    derived_dict = {}
    for d in deriveds:
...
def make_bundled_scope(schema: Schema, scope: CPPScope, bundle: dict) -> CPPScope:
    ...
    bscope = CPPScope(f'bundled_{scope.name}', [], [], [], scope.exec, scope.weights)
