...
private:
    ...
    int pick_scope(int* suitable, size_t n) {
        std::vector<int> pool;
    ...
    bool MutateContext() {
        ...
        int funcs[with_context.size()];
        for (size_t i=0; i < with_context.size(); ++i) {
            funcs[i] = with_context[i];
        }
    ...
    void MutateExtendConstructor() {
        ...
        int funcs[constructors.size()];
        for (size_t i=0; i < constructors.size(); ++i) {
            funcs[i] = constructors[i];
        }
