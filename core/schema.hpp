...
struct ScopeDef {
    ...
    std::unordered_map<std::string, unsigned int> weights; /* {"memset": 0, "free":1}  ; for default destructor is {} */
};
...
ScopeDef MakeScopeDef(json scope, unsigned int index) {
    ...
    auto _weights = scope["weights"];
    
    std::cout << _weights << std::endl;
    for (auto op_weight : _weights.items()) {
        def.weights[(std::string)op_weight.key()] = (unsigned int)op_weight.value();
    }
...
private:
    ...
    //curr_class : [parents]
    std::unordered_map<unsigned int, std::vector<unsigned int>> bases; 
    //curr_class : [children]
    std::unordered_map<unsigned int, std::vector<unsigned int>> deriveds; 

    // {type_id: type_idx}
    std::unordered_map<unsigned int, unsigned int> type_remap;

    std::unordered_map<unsigned int, std::vector<TypeTree>> initializers;
    std::unordered_map<unsigned int, unsigned int> init_map; // index in cache["initializers"] : type_id
...
public:
     ...
     * }
     */
    void SaveCached(std::string filename, std::string filendex) {
        json cache;
        json ndex; // type id : [index of initializers in .json file]
        unsigned int n = 0;

        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            unsigned int type_id = (unsigned int)_schema["types"][i]["id"];

            for (size_t j = 0; j < initializers[type_id].size(); ++j) {
                cache["initializers"].push_back(TypeTreeToJSON(initializers[type_id][j]));
                ndex[type_id].push_back(n);
                n++;
            }
        ...
        out << cache << std::endl;
        std::ofstream out_(filendex);
        out_ << ndex << std::endl;
        // out << "produced by savecached" << std::endl;
    }

    void LoadCached(std::string filename, std::string filendex) {
        ...
        // Since each type might have more than 1 initializer now
        std::ifstream inp_(filendex);
        json ndex;
        inp_ >> ndex;
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            unsigned int type_id = (unsigned int)_schema["types"][i]["id"];
            for (size_t j = 0; j < ndex[type_id].size(); ++j) {
                unsigned int x = ndex[type_id][j];
                initializers[type_id].push_back(TypeTreeFromJSON(cache["initializers"][x]));
            }
        }
    }
    ...
    void CacheScopes(bool filter_usable = false) {
        ...
        // type_id of subclass : [parent classes]
        for (size_t i = 0; i < _schema["bases"].size(); ++i) {
            unsigned int type_id = (unsigned int)_schema["bases"][i]["id"];
            bases[type_id] = std::vector<unsigned int>();
            for (size_t j = 0; j < _schema["bases"][i]["child"].size(); ++j) {
                bases[type_id].push_back((unsigned int)_schema["bases"][i]["child"][j]);
            }
        }

        // type_id of baseclass : [subclasses]
        // std::unordered_map<unsigned int, std::vector<unsigned int>> deriveds;
        for (size_t i = 0; i < _schema["deriveds"].size(); ++i) {
            unsigned int type_id = (unsigned int)_schema["deriveds"][i]["id"];
            deriveds[type_id] = std::vector<unsigned int>();
            for (size_t j = 0; j < _schema["deriveds"][i]["parent"].size(); ++j) {
                deriveds[type_id].push_back((unsigned int)_schema["deriveds"][i]["parent"][j]);
            }
        }
        ...
        for (size_t i = 0; i < _schema["scopes"].size(); ++i) {
            ...
            for (unsigned int conn_idx = 0; conn_idx < def.in_ref.size(); ++conn_idx) {
                ...
                // If def.in_ref[conn_idx] has subclass
                //  consumers[subclass] add (def, conn_idx)
                // conn_idx no change since this indicates which argument of that def ie. func is it consuming ??
                unsigned int input_type = def.in_ref[conn_idx];
                std::vector<unsigned int> sub_cls = deriveds[input_type];
                for (int i = 0; i < sub_cls.size(); ++i) {
                    consumers[sub_cls[i]].push_back(std::make_tuple(def, conn_idx));
                }
            }
            ...
            for (unsigned int conn_idx = 0; conn_idx < def.out_ref.size(); ++conn_idx) {
                ...
                // If def.out_ref[conn_idx] has superclass
                // producers[superclass]add (def, conn_idx)
                unsigned int output_type = def.out_ref[conn_idx];
                std::vector<unsigned int> super_cls = bases[output_type];
                for (int i = 0; i < super_cls.size(); ++i) {
                    producers[super_cls[i]].push_back(std::make_tuple(def, conn_idx));
                }
            }
    ...
    void helper(unsigned int type_id, std::vector<int> done) {
        std::cout << done[type_id] << " entering helper for " << type_id << std::endl;
        std::cout << &deriveds << std::endl;
        if (done[type_id] != 1) {
            std::vector<unsigned int> sub = deriveds[type_id]; //subclasses
            std::cout << deriveds[type_id].size() << " subclasses for base " << type_id << std::endl;
            // make sure all subclasses done
            for (size_t i=0; i < sub.size(); ++i) {
                unsigned int s = sub[i];
                std::cout << s << std::endl;
                helper(s, done);
            }
            // add subclass' init 
            for (size_t i=0; i < sub.size(); ++i) {
                unsigned int s = sub[i];
                for (size_t j=0; j < initializers[s].size(); ++j) {
                    initializers[type_id].push_back(initializers[s][j]);
                } 
            }
        }
        done[type_id] = 1;
        std::cout << type_id << " is done" << std::endl;
    }
    ...
    void CacheTypeTrees(bool debug=false, unsigned int max_depth=5) {
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            ...
            initializers[type_id].push_back(BuildTypeTree(type_id, true, debug, max_depth));
            finalizers[type_id] = BuildTypeTree(type_id, false, debug, max_depth);

            if (debug) {
                std::cerr << "Tree for: " << _schema["types"][i]["name"] << std::endl;
                PrintTypeTree(initializers[type_id][0]);
                PrintTypeTree(finalizers[type_id]);
            }
        }

        std::vector<int> _done; //start from all not added
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            _done.push_back(0);
        }
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
    ...
    bool Validate(bool debug=false, unsigned int max_depth=5, bool skip_validation=false, bool prune_cache=false) {
        ...
        for (size_t i = 0; i < _schema["types"].size(); ++i) {
            ...
            // Just test the first one, which is the one generated by original implementation, not the ones that belong to subclass and added to baseclass
            if (initializers[type_id][0].num_subtrees == 0) {
    ...
    TypeTree GetInitializer(unsigned int type) {
        return initializers[type][rand() % initializers[type].size()];
    ...
    int con_des(const char* filepath) {
        ...
        // Write the contents of the is_constructor vector to the file
        outfile << "IS_CONSTRUCTOR" << std::endl;

        for (int i=0; i<8; ++i) {
            outfile << IsConstructor(i) << ", ";
        }
        outfile << "NEXT" << std::endl;

        for (bool b : is_constructor) {
            outfile << b << ", ";
        }
        outfile << std::endl;
        // Write the contents of the is_destructor vector to the file
        outfile << "IS_DESTRUCTOR" << std::endl;
        for (bool b : is_destructor) {
            outfile << b << ", ";
        }
        outfile << std::endl;
        outfile << std::endl;
        outfile << "CONSUMERS";
        // Iterate through consumers unordered_map
        // std::unordered_map<unsigned int, std::vector<std::tuple<ScopeDef, unsigned int>>>:: iterator p; 
        // for (p = consumers.begin();  p != consumers.end(); p++) {
        for (auto& map_pair : consumers) {
            outfile << std::endl;
            outfile << "Type id: " << map_pair.first << std::endl;
            for (auto& scp_connidx : map_pair.second) {
                outfile << std::get<0>(scp_connidx).name << " " << std::get<1>(scp_connidx) << "  ,  ";
            }
        }
        outfile << std::endl;
        outfile << std::endl;
        ...
        // Check if the file stream was successfully closed
        if (outfile.fail()) {
            std::cerr << "Error: Could not close the file: " << filepath << std::endl;
            return 1; // Return a non-zero value to indicate an error
        }
