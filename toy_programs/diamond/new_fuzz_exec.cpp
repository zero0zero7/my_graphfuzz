...
template <class B, class D, class SubB> void __attribute__((visibility ("default"))) ambi_cast_helper(void** in, void** out) {
    D* d = reinterpret_cast<D*>(in[0]);
    // B* b = (B*)d;
    SubB* i = (SubB*)d;
    B* b = (B*)i;
...
//     if ((b_id)==0 && (d_id)==4) {(((*(cast_map))[(b_id)])[(d_id)]) = (ambi_cast_helper<Base,Derived, C>);} \
//     else {(((*(cast_map))[(b_id)])[(d_id)]) = (cast_helper<Base,Derived>);} \
// })
#define BASE2DERIVED(Base, Derived, SubBase) { \
    unsigned b_id = name_id[#Base], \
        d_id = name_id[#Derived]; \
    /* remembers the instantiated cast */ \
    (*cast_map)[b_id][d_id] = ambi_cast_helper<Base,Derived, SubBase>; \
}
